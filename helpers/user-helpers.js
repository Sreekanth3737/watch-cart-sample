...
const getCartProducts=(userId)=>{
    return new Promise(async(resolve,reject)=>{
        let cartItems=await Cart.find({user:userId}).lean().populate({path:'cartItems.product',populate:{path:'brandnames'}}).populate({path:'cartItems.product',populate:{path:'subcategory'}}).populate({path:'cartItems.product',populate:{path:'catagory'}})
...
verifyPayment:(details)=>{
    ...
    return new Promise(async(resolve,reject)=>{
        ...
        let hash = createHmac('sha256',process.env.RAZORPAY_KEY)
